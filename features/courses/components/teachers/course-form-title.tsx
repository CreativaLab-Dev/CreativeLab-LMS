...
import { updateCourseTitle } from "../../actions/teachers/update-course-title";
import { toast } from "sonner";

interface CourseFormTitleProps {
  initialData: {
    title: string;
  };
  courseId: string;
...
const CourseFormTitle = ({ courseId, initialData }: CourseFormTitleProps) => {
  ...
  const onSubmit = async (data: z.infer<typeof EditCourseTitleSchema>) => {
    startTransition(() => {
      updateCourseTitle(courseId, data)
        .then((response) => {
          if (response.success) {
            setIsEditting(false);
