...
import { EditMembershipFormSchema } from "../schemas"
...
const MembershipEditForm = ({
  ...
  const onSubmit = (values: z.infer<typeof EditMembershipFormSchema>) => {
    if (isPending) return
    startTransition(() => {
      updateMembership(membership.id, values).then((result) => {
        if (result?.success) {
    ...
    <div className="max-w-5xl p-6 md:p-10">
      ...
      <div>
        ...
        <p className="text-sm text-slate-600">
          Edita la membresía de un usuario
        </p>
        <Form {...form}>
          <form
